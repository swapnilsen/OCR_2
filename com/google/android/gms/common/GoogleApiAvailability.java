...
public class GoogleApiAvailability extends zze {
    ...
    public Dialog zza(Activity activity, OnCancelListener onCancelListener) {
        ...
        builder.setPositiveButton(FirebaseRemoteConfig.DEFAULT_VALUE_FOR_STRING, null);
    ...
    void zza(Context context, int i, String str, PendingIntent pendingIntent) {
        if (i == 18) {
            ...
            if (zzj.zzba(context)) {
                zzac.zzaw(zzt.zzzm());
                build = new Notification.Builder(context).setSmallIcon(context.getApplicationInfo().icon).setPriority(2).setAutoCancel(true).setContentTitle(zzh).setStyle(new BigTextStyle().bigText(zzj)).addAction(C0902R.drawable.common_full_open_on_phone, resources.getString(C0902R.string.common_open_on_phone), pendingIntent).build();
            } else {
                build = new NotificationCompat.Builder(context).setSmallIcon(17301642).setTicker(resources.getString(C0902R.string.common_google_play_services_notification_ticker)).setWhen(System.currentTimeMillis()).setAutoCancel(true).setContentIntent(pendingIntent).setContentTitle(zzh).setContentText(zzj).setLocalOnly(true).setStyle(new NotificationCompat.BigTextStyle().bigText(zzj)).build();
            }
            switch (i) {
                case NendIconError.ERROR_ICONVIEW /*1*/:
